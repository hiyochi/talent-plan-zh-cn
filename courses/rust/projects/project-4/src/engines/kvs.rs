...
/// `KvStore` 存储字符串键值对。
///
/// 键值对持久化到磁盘上的日志文件中。日志文件以单调递增的生成编号命名，扩展名为 `log`。
/// 内存中的跳表存储键和值的位置，以实现快速查询。
///
/// ```rust
...
impl KvsEngine for KvStore {
    ...
    /// # 错误
    ///
    /// 如果给定键未找到，则返回 `KvsError::KeyNotFound`。
    ///
    /// 在写入日志时，会传播 I/O 或序列化错误。
...
/// 单线程读取器。
///
/// 每个 `KvStore` 实例都有其自己的 `KvStoreReader`，
/// 并且 `KvStoreReader` 会分别打开相同的文件。因此用户
/// 可以通过不同线程中的多个 `KvStore` 并发读取。
...
impl KvStoreReader {
    ...
    ///
    /// `safe_point` 在压缩完成后更新为最新的压缩生成编号。
    /// 压缩生成编号包含其之前的所有操作，且内存索引中不包含生成编号小于 safe_point 的条目。
    /// 因此我们可以安全地关闭这些文件句柄，过时的文件可以被删除。
        ...
        F: FnOnce(io::Take<&mut BufReaderWithPos<File>>) -> Result<R>,
    {
        ...
        let mut readers = self.readers.borrow_mut();
        // 如果我们尚未在此 `KvStoreReader` 中打开该文件，则打开它。
...
/// 使用给定生成编号创建一个新的日志文件，并将读取器添加到 readers 映射中。
///
/// 返回日志的写入器。
